...
def call_grpc(request, response_func, method):
    ...
    def _f(f):
        ...
        async def call(self, *args, timeout=None, **kwargs):
            r = self.grpc_call(method(self), request(*args, **kwargs), timeout=timeout)
...
class Auth(StubMixin):
    def _update_channel(self, channel):
        ...
        self._auth_stub = rpc.KVStub(channel)

    @call_grpc(lambda: rpc.AuthEnableRequest(), lambda r: None, lambda s: s._auth_stub)
    async def auth_enable(self):
        pass

    @call_grpc(lambda: rpc.AuthDisableRequest(), lambda r: None, lambda s: s._auth_stub)
    async def auth_disable(self):
        pass

    @call_grpc(lambda u, p: rpc.AuthenticateRequest(name=u, password=p), lambda r: r.token, lambda s: s._auth_stub)
    async def authenticate(self, username, password):
        pass

    @call_grpc(lambda: rpc.AuthUserListRequest(), lambda r: [u for u in r.users], lambda s: s._auth_stub)
    async def user_list(self):
        pass

    @call_grpc(lambda u, p: rpc.AuthUserAddRequest(name=u, password=p), lambda r: None, lambda s: s._auth_stub)
    async def user_add(self, username, password):
        pass

    @call_grpc(lambda u: rpc.AuthUserDeleteRequest(name=u), lambda r: None, lambda s: s._auth_stub)
    ...
    @call_grpc(lambda u, p: rpc.AuthUserChangePasswordRequest(name=u, password=p), lambda r: None,
               lambda s: s._auth_stub)
    async def user_change_password(self, username, password):
        pass

    @call_grpc(lambda u, r: rpc.AuthUserGrantRoleRequest(name=u, role=r), lambda r: None, lambda s: s._auth_stub)
    async def user_grant_role(self, username, role):
        pass

    @call_grpc(lambda u, r: rpc.AuthUserRevokeRoleRequest(name=u, role=r), lambda r: None, lambda s: s._auth_stub)
    async def user_revoke_role(self, username, role):
        pass

    @call_grpc(lambda: rpc.AuthRoleListRequest(), lambda r: [role for role in r.roles], lambda s: s._auth_stub)
    async def role_list(self):
        pass

    @call_grpc(lambda role: rpc.AuthRoleGetRequest(role=role), lambda r: [p for p in r.perm], lambda s: s._auth_stub)
    async def role_get(self, name):
        pass

    @call_grpc(lambda role: rpc.AuthRoleAddRequest(name=role), lambda r: None, lambda s: s._auth_stub)
    async def role_add(self, name):
        pass

    @call_grpc(lambda role: rpc.AuthRoleDeleteRequest(name=role), lambda r: None, lambda s: s._auth_stub)
    ...
    @call_grpc(role_grant_request, lambda r: None, lambda s: s._auth_stub)
    ...
    @call_grpc(role_revoke_request, lambda r: None, lambda s: s._auth_stub)
